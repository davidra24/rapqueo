    ...
    <div v-else>
      <md-drawer class="md-right" :md-active.sync="openedCart">
        ...
        <md-list class="d-flex justify-content-center col-12">
          ...
          <md-list-item style v-show="getCountCart > 0">Subtotal: ${{totalPriece}}</md-list-item>
          <md-list-item
            style="margin-top:-7%;"
            v-show="getCountCart > 0"
          >Domicilio: ${{aditional.precio}}</md-list-item>
          <md-list-item v-show="getCountCart > 0">
            <h6>
              <strong>Precio Total:</strong>
              ${{ totalPriece + this.aditional.precio }}
            </h6>
          </md-list-item>
          <md-list-item v-show="getCountCart > 0">
...
export default {
  ...
  methods: {
    ...
    fetch(id) {
      getOneOrManyApi(aditionals, id)
        .then(res => {
          this.setAditional(res.data);
          this.add = false;
        })
        .catch(err => {
          this.setError(err);
